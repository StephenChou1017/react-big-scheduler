...
async function build() {
  ...
  try {
    ...
    process.stdout.write(
      `Copying library style definitions to ${cssTarget}/style.css ... \n`
    );
    const cssResult = await exec(
      `mkdir -p ${cssTarget} && cp ${sourceDir}/css/style.css ${cssTarget}/style.css`
    );

    // copy types
    process.stdout.write(
      `Copying type definitions to ${jsTarget}/index.d.ts ... \n`
    );
    const tsResult = await exec(
      `cp ${sourceDir}/index.d.ts ${jsTarget}/index.d.ts`
    );
